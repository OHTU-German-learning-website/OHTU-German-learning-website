...
export default function JumbledSentenceExercisesPage() {
  const {
    ...
    error,
    refetch,
  } = useQuery("/exercises/jumbled-sentence");
  ...
  const handleDelete = async (exerciseId) => {
    ...
    try {
      const response = await fetch(
        withBasePath(`/api/admin/exercises/jumbled-sentence/${exerciseId}`),
        { method: "DELETE" }
      );
  ...
  if (!exercises || exercises.length === 0) {
    return (
      <Container maxW="800px" m="0 auto" p="md">
        <Container mb="lg">
          <h1>Jumbled Sentence Übungen</h1>
        </Container>
        ...
        {exercises.map((exercise) => (
          ...
          >
            <Column flex="1">
              ...
              >
                <Link
                  href={`/grammar/exercises/jumbled-sentence/${exercise.id}`}
            ...
            <Column gap="sm" align="flex-end">
              ...
              <Link
                href={`/admin/create-exercise/jumbled-sentence/${exercise.id}/edit`}
              >
